...
namespace DiskInfo{
		...
		ATASMART::ATASMART(const std::string _device){
				...
				sk_disk_smart_parse_attributes(disk,
		...
		uint64_t ATASMART::powerOnTime() const{
				if(std::get<0>(powerOnTime_)) return std::get<1>(powerOnTime_);
				else return 0xFFFFFFFF;
		}

		uint64_t ATASMART::temperature() const{
				if(std::get<0>(temperature_)) return std::get<1>(temperature_);
				else return 0xFFFFFFFF;
		}

		uint64_t ATASMART::powerOnCount() const{
				if(std::get<0>(powerOnCount_)) return std::get<1>(powerOnCount_);
				else return 0xFFFFFFFF;
		}

		uint64_t ATASMART::capacity() const{
				if(std::get<0>(capacity_)) return std::get<1>(capacity_);
				else return 0xFFFFFFFF;
		}
