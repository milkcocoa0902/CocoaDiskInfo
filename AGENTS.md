...
- `strategy/`: design direction and implementation strategy documents.

## 3. Strategy Documents
- Treat `strategy/0006_agent_evolution_direction.md` as the top-level direction document for agent evolution.
- Before making architecture, mode, storage, API, client history, packaging, or Prometheus-related changes, read `0006` first and follow its decision rules.
- Use the other `strategy/` documents as supporting, topic-specific plans.
- If a proposed change does not fit the direction in `0006`, write or update a strategy document before implementation.
...
- Follow `strategy/0007_db_data_lifetime_policy.md` when changing retention, cleanup, history, cache, or event storage behavior.
